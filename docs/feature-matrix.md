...
| **Computer-use** | computer-use repos, Mark-XLVI | Full computer control: planner + executor + agentic tools (shell, filesystem, browser, system). Unrestricted capability, trust-gated approval, audit/undo logs, continuous vision (moondream + pytesseract), audio context | `services/core-api/app/{services/planner/,services/tools/,services/vision/,services/audio/,models/plan.py,routers/plans.py}` | P1→P2 | implemented |
...
| **Execution Plans** | computer-use repos, Odysseus | TaskPlan/PlanStep DB models, planner (LLM decomposition), executor (parallel batches, approval, replan, double-verify), plan CRUD API, WS lifecycle events | `services/core-api/app/{models/plan.py, routers/plans.py, services/planner/}` | P1→P2 | implemented |
...
| **Multi-agent orchestration** | Odysseus | PlannerService + ExecutorService with parallel batches, replan, double-verification, and LLM-driven step decomposition | `services/core-api/app/services/planner/` · `services/core-api/app/models/plan.py` · `services/core-api/app/routers/plans.py` | P2 | implemented |
...
- **Voice (STT/TTS)** is implemented via OpenAI Whisper/TTS with mock fallback.
- **Computer-use** is implemented: full agentic tools (shell, fs, browser, system), planner/executor, trust levels, audit/undo logs, continuous vision, audio context, and plan CRUD/WS events.
